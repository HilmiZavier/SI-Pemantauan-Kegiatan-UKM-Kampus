...
<div class="page-header">
    ...
    <p class="page-subtitle">Silakan isi formulir di bawah ini untuk menambahkan akun pengguna baru ke sistem.</p>
</div>

<div class="card-premium" style="max-width: 650px;">
    <div class="card-premium-header">
        <h5 class="fw-bold m-0"><i class="bi bi-person-plus-fill me-2 text-primary"></i>Formulir Data User</h5>
    </div>
    <div class="card-premium-body">
        <form action="<?= base_url('user/store') ?>" method="post">
            ...
            <div class="form-premium-group">
                ...
                <input type="email" name="email" id="email" class="form-control form-premium-control w-100 <?= session('errors.email') ? 'is-invalid' : '' ?>" value="<?= old('email') ?>" required placeholder="user@unitomo.ac.id">
            ...
            <div class="form-premium-group">
                <label for="password" class="form-premium-label">Kata Sandi (Password)</label>
                <input type="password" name="password" id="password" class="form-control form-premium-control w-100 <?= session('errors.password') ? 'is-invalid' : '' ?>" required placeholder="Minimal 6 karakter">
            ...
            <div class="form-premium-group">
                ...
                <select name="role_id" id="role_id" class="form-select form-premium-control w-100" required>
                    ...
                    <?php foreach ($roles as $role): ?>
                        <option value="<?= $role['id'] ?>" <?= old('role_id') == $role['id'] ? 'selected' : '' ?>><?= esc($role['nama_role']) ?></option>
                    <?php endforeach; ?>
                </select>
            ...
            <div class="form-premium-group" id="ukm-select-group" style="display: none;">
                ...
                <select name="ukm_id" id="ukm_id" class="form-select form-premium-control w-100">
                    ...
                    <?php foreach ($ukms as $ukm): ?>
                        <option value="<?= $ukm['id'] ?>" <?= old('ukm_id') == $ukm['id'] ? 'selected' : '' ?>><?= esc($ukm['nama_ukm']) ?></option>
                    <?php endforeach; ?>
                </select>
...
<script>
    ...
    function toggleUkmSelect() {
        // Jika peran yang dipilih adalah Admin UKM (ID = 1), tampilkan pilihan UKM
    ...
    // Panggil saat halaman dimuat (jika ada input lama)
    toggleUkmSelect();
</script>
<?= $this->endSection() ?>
